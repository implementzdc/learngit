(function ($) {
    $.learuntab = {
        ...
        activeTab: function () {
            ...
            $.session.set('menuUrl', currentId);//存入模块Url
        ...
        addTab: function () {
            ...
            if (flag) {
            	//$.session.set('menuid', dataId);//存入模块ID
            	$.session.set('menuUrl', dataUrl);//存入模块url
    ...
    $.learunindex = {
        ...
        loadMenu: function () {
            var data;
            $.ajax({
            	   url: "/getNewmenu.do",//json文件位置
            	   type: "POST",//请求方式为get
            	   async: false,	
            	   dataType: "json", //返回数据格式为json
            	   success: function(tmp) {//请求成功完成后要执行的方法 
            			data=tmp.menu;
            	   }
            	});
            var _html = "";
            $.each(data, function (i) {
                ...
                if (row.parentid == "0") {
                    ...
                    _html += '<a href="#">'
                    _html += '<i class="fa fa-desktop"></i><span>' + row.description + '</span><i class="fa fa-angle-left pull-right"></i>'
                    _html += '</a>'
                    var childNodes = $.learunindex.jsonWhere(data, function (v) { return v.parentid == row.num });
                    if (childNodes.length > 0) {
                        _html += '<ul class="treeview-menu">';
                        $.each(childNodes, function (i) {
                            var subrow = childNodes[i];
                            var subchildNodes = $.learunindex.jsonWhere(data, function (v) { return v.parentid == subrow.num });
                            _html += '<li>';
                            if (subchildNodes.length > 0) {
                                _html += '<a href="#"><i class="fa fa-desktop"></i>' + subrow.description + '';
                                _html += '<i class="fa fa-angle-left pull-right"></i></a>';
                                _html += '<ul class="treeview-menu">';
                                $.each(subchildNodes, function (i) {
                                    var subchildNodesrow = subchildNodes[i];
                                    _html += '<li><a class="menuItem" data-id="' + subrow.num + '" href="' + subrow.url + '"><i class="' + subchildNodesrow.F_Icon + '"></i>' + subchildNodesrow.description + '</a></li>';
                                });
                                _html += '</ul>';

                            } else {
                                _html += '<a class="menuItem" data-id="' + subrow.num + '" href="' + subrow.url + '"><i class="fa fa-desktop"></i>' + subrow.description + '</a>';
                            }
                            _html += '</li>';
                        });
                        _html += '</ul>';
